...
class OrderController extends Controller
{
    ...
    public function detail(Request $request)
    {
        ...
        if (!$order) abort(500, 'Order does not exist');
    ...
    public function paid(Request $request)
    {
        ...
        if ($order->status !== 0) abort(500, 'Can only be operated on pending orders');

        $orderService = new OrderService($order);
        if (!$orderService->paid('manual_operation')) {
            abort(500, 'Update failure');
        }
        return response([
            'data' => true
        ]);
    ...
    public function assign(OrderAssign $request)
    {
        ...
        if ($userService->isNotCompleteOrderByUserId($user->id)) {
            abort(500, 'This user has pending orders that cannot be assigned');
        }
        ...
        if (!$order->save()) {
            DB::rollback();
            abort(500, 'Order creation failure');
        }
